...
public class SeedKisa {
	...
	public byte[] encrypt(byte[] sbuffer, byte szKey[]) {
		...
		byte[] inDataBuffer = addPadding(sbuffer, SeedBlockSize);
	...
	private static int SS0[] = {
		...
		0x2585a1a4, 0x0f8f838c, 0x03030300, 0x3b4b7378, 0x3b9bb3b8, 0x13031310, 0x12c2d2d0, 0x2ecee2ec,
	...
	private static int SS2[] = {
		...
		0xf3fc3fcf, 0x41480949, 0x31383909, 0x63642747, 0xc0c000c0, 0xc3cc0fcf, 0xd3d417c7, 0xb0b8e888,
	...
	private static int SS3[] = {
		...
		0x9fa3ac2f, 0x40606020, 0x45515415, 0xc7c3c407, 0x44404404, 0x4f636c2f, 0x4b63682b, 0x4b53581b,
		...
		0xc7f3f437, 0x8da1ac2d, 0x90909000, 0x0f131c1f, 0xcac2c80a, 0x0c202c2c, 0x8aa2a82a, 0x04303434,
		0xc2d2d012, 0x0b03080b, 0xcee2ec2e, 0xc9e1e829, 0x4d515c1d, 0x84909414, 0x08101818, 0xc8f0f838,
		0x47535417, 0x8ea2ac2e, 0x08000808, 0xc5c1c405, 0x03131013, 0xcdc1cc0d, 0x86828406, 0x89b1b839,
		0xcff3fc3f, 0x4d7a7c3d, 0xc1c1c001, 0x01313031, 0xc5f1f435, 0x8a82880a, 0x4a62682a, 0x81b1b031,
		0xc1d1d011, 0x00202020, 0xc7d3d417, 0x02020002, 0x02222022, 0x04000404, 0x47506828, 0x41717031,
		0x07030407, 0xcbd3d81b, 0x8d919c1d, 0x89919819, 0x41616021, 0x8eb2bc3e, 0xc6e2e426, 0x49515819,
		0xcdd1dc1d, 0x41515011, 0x80909010, 0xccd0dc1c, 0x8a92981a, 0x83a3a023, 0x8ba3a82b, 0xc0d0d010,
		0x81818001, 0x0f030c0f, 0x47434407, 0x0a12181a, 0xc3e3e023, 0xcce0ec2c, 0x8d818c0d, 0x8fb3bc3f,
		0x86929416, 0x4b73783b, 0x4c505c1c, 0x82a2a022, 0x81a1a02a, 0x43636023, 0x03232023, 0x4d414c0d,
		...
		0xc9d1d8a9, 0x4c404c0c, 0x83838003, 0x8f838c0f, 0xcec2cc0e, 0x0b33383b, 0x4a42480a, 0x87b3b437
	};
	...
	// private satic Boolean ENDIAN = LITLE;
					...
					byte outData[]) 		// [out] encrypted data
	{
		...
		L0[0] = ((L0[0])<<8)^((int)pbData[1]&0x0000ff);
		L0[0] = ((L0[0])<<8)^((int)pbData[2]&0x0000ff);
		L0[0] = ((L0[0])<<8)^((int)pbData[3]&0x0000ff);
		
		L1[0] = ((int)pbData[4]&0x000000ff);
		L1[0] = ((L1[0])<<8)^((int)pbData[5]&0x0000ff);
		L1[0] = ((L1[0])<<8)^((int)pbData[6]&0x0000ff);
		L1[0] = ((L1[0])<<8)^((int)pbData[7]&0x0000ff);
		
		R0[0] = ((int)pbData[8]&0x000000ff);
		R0[0] = ((R0[0])<<8)^((int)pbData[9]&0x0000ff);
		R0[0] = ((R0[0])<<8)^((int)pbData[10]&0x0000ff);
		R0[0] = ((R0[0])<<8)^((int)pbData[11]&0x0000ff);
		
		R1[0] = ((int)pbData[12]&0x000000ff);
		R1[0] = ((R1[0])<<8)^((int)pbData[13]&0x0000ff);
		R1[0] = ((R1[0])<<8)^((int)pbData[14]&0x0000ff);
		R1[0] = ((R1[0])<<8)^((int)pbData[15]&0x0000ff);
						...
						byte outData[])				// [out] data to be encrypted
	{
		...
		int R0[] = new int[1];		int R1[] = new int[1];
		int K[] = new int[2];
		L0[0] = 0x00;		L1[0] = 0x0; 	R0[0] = 0x00; 	R1[0] = 0x0;
		int nCount = 3;
	...
	public static void SeedRoundKey(			
			...
			byte pbUserKey[])				// [in] data to be encrypted
	{
		int A[] = new int[1];		int B[] = new int[1];
		int C[] = new int[1];		int D[] = new int[1];
		int K[] = new int[2];
		int T0, T1;
		int nCount = 2;
		...
		A[0] = ((A[0])<<8)^((int)pbUserKey[1]&0x0000ff);
		A[0] = ((A[0])<<8)^((int)pbUserKey[2]&0x0000ff);
		A[0] = ((A[0])<<8)^((int)pbUserKey[3]&0x0000ff);
		
		B[0] = ((int)pbUserKey[4]&0x000000ff);
		B[0] = ((B[0])<<8)^((int)pbUserKey[5]&0x0000ff);
		B[0] = ((B[0])<<8)^((int)pbUserKey[6]&0x0000ff);
		B[0] = ((B[0])<<8)^((int)pbUserKey[7]&0x0000ff);
		
		C[0] = ((int)pbUserKey[8]&0x000000ff);
		C[0] = ((C[0])<<8)^((int)pbUserKey[9]&0x0000ff);
		C[0] = ((C[0])<<8)^((int)pbUserKey[10]&0x0000ff);
		C[0] = ((C[0])<<8)^((int)pbUserKey[11]&0x0000ff);
		
		D[0] = ((int)pbUserKey[12]&0x000000ff);
		D[0] = ((D[0])<<8)^((int)pbUserKey[13]&0x0000ff);
		D[0] = ((D[0])<<8)^((int)pbUserKey[14]&0x0000ff);
		D[0] = ((D[0])<<8)^((int)pbUserKey[15]&0x0000ff);
		...
		EncRoundKeyUpdate0(K, A, B, C, D, 1);	pdwRoundKey[nCount++] = K[0];	pdwRoundKey[nCount++] = K[1];
		EncRoundKeyUpdate1(K, A, B, C, D, 2);	pdwRoundKey[nCount++] = K[0];	pdwRoundKey[nCount++] = K[1];
		EncRoundKeyUpdate0(K, A, B, C, D, 3);	pdwRoundKey[nCount++] = K[0];	pdwRoundKey[nCount++] = K[1];
		EncRoundKeyUpdate1(K, A, B, C, D, 4);	pdwRoundKey[nCount++] = K[0];	pdwRoundKey[nCount++] = K[1];
		EncRoundKeyUpdate0(K, A, B, C, D, 5);	pdwRoundKey[nCount++] = K[0];	pdwRoundKey[nCount++] = K[1];
		EncRoundKeyUpdate1(K, A, B, C, D, 6);	pdwRoundKey[nCount++] = K[0];	pdwRoundKey[nCount++] = K[1];
		EncRoundKeyUpdate0(K, A, B, C, D, 7);	pdwRoundKey[nCount++] = K[0];	pdwRoundKey[nCount++] = K[1];
		EncRoundKeyUpdate1(K, A, B, C, D, 8);	pdwRoundKey[nCount++] = K[0];	pdwRoundKey[nCount++] = K[1];
		EncRoundKeyUpdate0(K, A, B, C, D, 9);	pdwRoundKey[nCount++] = K[0];	pdwRoundKey[nCount++] = K[1];
		EncRoundKeyUpdate1(K, A, B, C, D, 10);	pdwRoundKey[nCount++] = K[0];	pdwRoundKey[nCount++] = K[1];
		EncRoundKeyUpdate0(K, A, B, C, D, 11);	pdwRoundKey[nCount++] = K[0];	pdwRoundKey[nCount++] = K[1];
		EncRoundKeyUpdate1(K, A, B, C, D, 12);	pdwRoundKey[nCount++] = K[0];	pdwRoundKey[nCount++] = K[1];
		EncRoundKeyUpdate0(K, A, B, C, D, 13);	pdwRoundKey[nCount++] = K[0];	pdwRoundKey[nCount++] = K[1];
		EncRoundKeyUpdate1(K, A, B, C, D, 14);	pdwRoundKey[nCount++] = K[0];	pdwRoundKey[nCount++] = K[1];
		EncRoundKeyUpdate1(K, A, B, C, D, 15);	pdwRoundKey[nCount++] = K[0];	pdwRoundKey[nCount++] = K[1];
	}
	...
	public byte[] decrypt(byte[] encryptBytes, byte[] szKey) {
		...
		return removePadding(decryptBytes, SeedBlockSize);
	}
	
	/**
	 * 요청한 Block Size 를 맞추기 위해 추가 된 Padding 을 제거한다.
	 * 
	 * @param source
	 * 			byte[] 패딩을 제거할 bytes
	 * @param blockSize
	...
	public byte[] removePadding(byte[] source, int blockSize) {
		...
		if(lastValue < (blockSize - 1)) {
			...
			for(int i=2; i < (zeroPaddingCount + 2); i++) {
				if(source[source.length - 1] != PADDING_VALUE) {
					isPadding = false;
					break;
				}
			}
			...
			// 마지막 값이 block size 보다 클 경우 패딩 된 것이 없음.
			isPadding = true;
		}
	...
	private void SeedEncRoundKey(int pdwRoundKey[], byte pbUserKey[]) {
		...
		int T0 = (int)A[0] + (int)C[0] - (int)KC[0];
		int T1 = (int)B[0] - (int)D[0] + (int)KC[0];
	...
	/** 
	 * 한국정보보호진흥원 소스에 없는 추가 된 함수로 암호화 된 바이트 데이터를 받아서 복호화하여 charset으로 문자열로 반환한다.
	 * 
	 * @param encryptBytes
	 * 			byte[] 암호화 된 바이트 데이터
	 * @param szKey
	 ...
	 * @return byte[] 복호화 된 바이트 데이터
	 */
	public String decryptAsString(byte[] encryptBytes, byte[] szKey, String charset) throws UnsupportedEncodingException {
		return new String(decrypt(encryptBytes, szKey), charset);
	}	
	
	/**
	 * 한국정보보호진흥원 소스에 없는 추가 된 함수로 입력 받는 문자열을 암호화하여 리턴한다.
	 * 
	 * @param inData
	 * 			String 평문
	 * @param szKey
	...
	/** 한국정보보호진흥원 소스에 없는 추가 된 함수로 입력 받은 문자열을 특정 Charset으로 변환하여 암호화하여 리턴한다.
	 * 
	 * @param inData
	 * 			String 평문
	 * @param szKey
	 * 			byte[] key
	 * @param charset
	 * 			String String 을 byte 데이터로 변환할때 사용할 charset
	 * @return byte[] 암호화 된 데이터
	 */
	...
	public byte[] addPadding(byte[] source, int blockSize) {
		...
		if(paddingCnt != 0) {
			...
			for(int i = 0; i < addPaddingCnt; i++) {
				paddingResult[source.length + 1] = PADDING_VALUE;
			}
	...
	public String nullTrim(String decryptText) {
